...
def test_da_set_up_application_round(student_preferences, school_priorities):
    da = DeferredAcceptance(
        student_preferences, school_priorities, np.ones(3, dtype=int)
    )
    current_rank = np.zeros(3, dtype=int)
    current_students = np.arange(3, dtype=int)
    current_applications, next_rank, next_students = da._set_up_next_application_round(
        current_rank, current_students
    )
...
def student_preferences_bigger():
    return np.array([[0, 1, 2, 3], [0, 3, 2, 1], [1, 0, 2, 3], [3, 1, 2, 0]])
...
def school_priorities_bigger():
    return (
        np.full((4, 4), 3)
        - np.array([[2, 3, 1, 0], [2, 0, 3, 1], [1, 2, 3, 0], [2, 1, 0, 3]]).T
    )


def test_da_get_priority_ordering(student_preferences_bigger, school_priorities_bigger):
    da = DeferredAcceptance(
        student_preferences_bigger, school_priorities_bigger, np.ones(4, dtype=int)
    )
...
def test_da_update_tentative_acceptances(
    student_preferences_bigger, school_priorities_bigger
):
    da = DeferredAcceptance(
        student_preferences_bigger, school_priorities_bigger, np.ones(4, dtype=int)
    )
    accepted, tentative_acceptances = da._update_tentative_acceptances(
...
def test_da_update_next_round_students(
    student_preferences_bigger, school_priorities_bigger
):
    da = DeferredAcceptance(
        student_preferences_bigger, school_priorities_bigger, np.ones(4, dtype=int)
    ...
    next_students, next_rank = da._update_next_round_students(
        accepted, next_rank, next_students, ordered_priority_index, 2, sch_applicants
    )
...
def test_da_larger_many_to_one(student_preferences_bigger, school_priorities_bigger):
    da = DeferredAcceptance(
        student_preferences_bigger, school_priorities_bigger, 2 * np.ones(4, dtype=int)
    )
    assignment = da.da()
